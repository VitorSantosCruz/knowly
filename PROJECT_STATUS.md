...
| `user-profile` | ✅ Done (mostly) | Frontend half of item 13 (profile view/edit UI), consuming `identity-profile-model`'s six shipped endpoints. New `ProfileService` (`core/profile.service.ts`, stateless HTTP wrapper, `ProfileFields`/`UserProfile`/`ProfileEditRequest` types) + `ProfileFieldsFormComponent` (shared, presentational, reused by both consumers below — never renders/emits `email`). `OwnProfilePageComponent` (`/profile`, new nav entry always visible once logged in, no guard — universal to any authenticated session per REQ-8): loads `GET /api/users/me/profile`; a `STAFF_ADMIN`/tenant-`ADMIN` session submits via direct `PUT`, applied immediately; anyone else submits via `POST .../edit-requests` and enters a client-side-only pending state (no backend "do I have a pending request" read exists — pending state is only set the first time a 409 is observed this session, an accepted rough edge carried over from PLAN.md's judgment call 4); both direct-edit and edit-request 409s show a non-technical conflict/already-pending message. `ProfileSectionComponent` (`features/user-management/profile-section.component.ts`) is a new, reusable `<section data-testid="profile-section">` wired into `StaffUserDetailPanelComponent` (after its audit-trail section) — own `profile`/`profileError` signals, a 403 renders `app-no-access-state` scoped to only that section, `[canEdit]` reveals an inline edit form reusing `ProfileFieldsFormComponent`. `ProfileEditRequestsInboxPageComponent` (`/profile-edit-requests`, nav-gated on tenant/global `PROFILE_EDIT` OR a tenant `ADMIN` membership OR `STAFF_ADMIN`-shaped, no route guard — the list endpoint never 403s, it just returns fewer/no rows) lists every pending request (`"User #{id}"` — no requester email/display name is in the shipped DTO, flagged as a known follow-up below), approve/reject remove the row on success, a 409 on approve keeps the row and shows a conflict message (REQ-15), a 403/stale 409 refreshes the list and shows the existing error state (REQ-16), zero rows shows a distinct empty state. New `Permission`/`GlobalPermission.PROFILE_EDIT` added to both frontend unions (backend-shipped, not previously mirrored). **Known, scoped gap (not shipped this iteration):** `ProfileSectionComponent` is *not* wired into `MemberDetailPanelComponent`/`MembersPageComponent` — verified during implementation that neither `MemberDto` nor `MemberDetailDto` (backend, shipped) expose a `userId` field, only `membershipId`, so there is no way for this frontend-only feature to resolve the target user id `GET /api/users/{id}/profile` needs without a backend contract change (ruled out of this SPEC's scope) — tracked as a follow-up requiring a small backend addition (e.g. a `userId` field on those two DTOs); see `knowly-app/specify/features/user-profile/PLAN.md`'s "Deviations from this PLAN" section. A second, smaller accepted rough edge: the edit-request inbox's nav-gating only reflects the *active* tenant's `PROFILE_EDIT` grant, not every tenant the caller might hold it in (same shape `nav.members`'s gating already uses). 321/321 frontend tests green, `format:check`/`build` clean, atomic commits per task. |
