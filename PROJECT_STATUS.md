...
**Current state (2026-08-09): `chat-message-search` (item 16) shipped
end-to-end on both sides** — backend (`GET /api/chat/messages/search`,
`ChatMessageSearchService`/`ChatMessageSearchRepository`, `V34` tsvector
migration, both AppSec-required regression tests passing) and frontend
(`ChatMessageSearchService`, `ChatSearchDialogComponent`, sidebar entry
point) — see both features' rows above for the full writeup. **Nothing
is currently queued next.** One candidate direction remains documented
...
| `chat-message-search` | ✅ Done (frontend) | Adds a Slack-style "search inside message content" surface on top of `chat-unified-ui`'s existing name-only directory search (that one untouched, per SPEC) — a new `GET /api/chat/messages/search` icon entry point, not a route or a fourth column. New `ChatMessageSearchService` (`core/chat-message-search.service.ts`, signals-based, mirrors `ChatDirectoryService`'s shape): `search()`/`loadMore()` build only the query params actually set (`q` always, `senderId`/`conversationId`/`dateFrom`/`dateTo` omitted entirely when unset — never sent as `null`/empty string), `loadMore()` appends to (not replaces) `_results` and derives `cursor` from the last response's `nextCursor`, and a failed `search()`/`loadMore()` leaves prior results untouched (REQ-13) rather than clearing them. `status` is an explicit `idle \| loading \| results \| no-results \| error` signal, not derived from `results().length` (REQ-12's distinct-state requirement — `no-results` and `idle` must never collapse into the same branch). New `ChatSearchDialogComponent` (native `<dialog>`, `create-group-dialog.component.ts`'s open/close precedent), opened from a new "Buscar mensagens" icon button in `ChatSidebarComponent` (`LucideSearch`), hosted by `ChatShellComponent` alongside its existing create-group/create-conversation dialogs. Free-text query is debounced 400ms (RxJS `Subject`+`debounceTime`+`distinctUntilChanged`, matching `tenant-pagination-search`'s existing debounce precedent) before firing a request; sender/conversation/date filter changes call `search()` immediately (no debounce — PLAN's explicit "only the per-keystroke free-text case needs one" distinction). REQ-7 (blank query)/REQ-8 (`dateFrom` after `dateTo`) are both blocked client-side before any HTTP call. Sender options come from `ChatService.eligibleParticipants()` plus every already-fetched conversation detail's `participantNicknames`, deduplicated by user id; conversation options come from `ChatService.conversations()` filtered to `PEER_DIRECT`/`PEER_GROUP` (`SUPPORT` excluded, matching the backend's own scope exclusion) — no new lookup endpoint for either. Results render via a new small presentational `ChatSearchResultRowComponent` (sender/conversation/timestamp/content, click and Enter/Space-keydown both emit `rowSelected` — renamed from the originally-planned `select`, which collides with the native DOM event name per `@angular-eslint/no-output-native`), pagination via both an `IntersectionObserver` sentinel and an explicit "Load more" button. **Scroll-to-message on result click is deliberately not implemented in v1** (PLAN.md's "Open dependency on backend feasibility work") — a result click opens the conversation at its normal newest-message view via `Router.navigate(['/chat', conversationId])`, with no `?highlight=` query param; a future increment needs a new backend "fetch page containing message X" endpoint that doesn't exist today. New `chat.search.*` i18n keys in both `en.json`/`pt-BR.json`. Built and tested against the API contract documented in `knowly-api/specify/features/chat-message-search/PLAN.md` (`ChatMessageSearchResultDto`/`ChatMessageSearchPageDto`) via `HttpTestingController` mocks, implemented in parallel with that backend feature — **not yet confirmed against the real running backend endpoint**; see that feature's own row/PROJECT_STATUS entry (or lack thereof, if it lands after this one) for backend-side status. 976/976 frontend tests green (20 new for this feature across its own spec files), `format:check`/`build`/`lint` all clean, 5 atomic commits (types, service, result-row component + i18n, dialog component, sidebar/shell wiring). |
