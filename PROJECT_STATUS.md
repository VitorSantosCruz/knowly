...
| `dashboard-analytics` | 🟡 Partially done — blocked | Period filter (`period-filter.component.ts`, PrimeNG `SelectButton`) owned by `dashboard-page.component.ts`'s `period` signal; `members-breakdown-card.component.ts` (`GET /api/tenants/metrics/members`); `top-articles-table.component.ts` (`p-table` + built-in global filter, replaces `article-usage-list.component.ts`); `export-button.component.ts` (CSV blob download via `GET /api/tenants/metrics/export?period=`); `metric-fetcher.ts`'s `load()` extended to accept `params`. **Blocked**: the SPEC/PLAN (both user-confirmed) assumed PrimeNG's `Chart` (`p-chart`) needs no new npm dependency, but `primeng/chart` does `import Chart from 'chart.js/auto'` and `chart.js` is not in `package.json`/`package-lock.json` at all — installing it is a genuine new external dependency (Tier 3 in `DECISIONS.md`, always-ask). Not implemented pending that decision: `metric-tile.component.ts` (all 5 sparkline tiles, supersedes `article-count-card`/`conversations-card`/`messages-card`), `message-split-chart.component.ts` (donut), `conversations-activity-chart.component.ts` (bar), and the accessibility `.sr-only` mirror tables for both. The three old point-in-time cards are therefore still in place (not yet superseded) since their replacement is blocked. `knowly-app/specify/features/dashboard-analytics/TASKS.md` reflects exactly which tasks are done/blocked. |
