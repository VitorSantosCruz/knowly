...
- **Known blocker (open, unrelated to observability, discovered
  2026-08-04)**: a fresh `./mvnw spring-boot:run` currently fails at
  startup with a `403 Forbidden` from MinIO on
  `ArticleStorageService#ensureBucketExists`'s `headBucket` call, even
  though `mc stat`/`mc ls` against the same MinIO container with the
  same credentials succeed. Bisected via `git log -p -- pom.xml` to
  `06d1fac` (Dependabot `software.amazon.awssdk:bom` bump 2.44.9→2.49.3,
  merged 2026-07-26) — a version range where AWS SDK v2 changed default
  request/response checksum behavior in ways known to break against
  older/self-hosted S3-compatible stores (this repo's pinned
  `minio/minio:RELEASE.2025-04-08T15-41-24Z`). The commonly-cited
  `AWS_REQUEST_CHECKSUM_CALCULATION`/
  `AWS_RESPONSE_CHECKSUM_VALIDATION=when_required` env-var workaround did
  **not** resolve it here — root cause needs dedicated triage. This
  blocks running the app locally at all (not just observability) until
  fixed or the AWS SDK bump is reverted/pinned back.
