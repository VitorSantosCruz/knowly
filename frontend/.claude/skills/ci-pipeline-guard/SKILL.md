...
- **STRICTLY PROHIBITED**: relaxing `reuseForks=false` or
  ...
  (documented in `../knowly/DECISIONS.md`) — these settings prevent a
...
- **DO** keep `compose.yaml` hardened: no hardcoded secrets
  ...
  unlike inside Spring's `application.yaml`, see `../knowly/DECISIONS.md`),
...
2. If touching Surefire config: read `../knowly/DECISIONS.md`'s Surefire entry
