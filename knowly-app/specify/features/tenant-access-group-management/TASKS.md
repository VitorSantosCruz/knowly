...
- [ ] 1.1. Write `member.service.spec.ts` case for
...
- [ ] 1.2. Implement `batchAssignAccessGroups` in `member.service.ts`
      (Green).
- [ ] 1.3. Write `member.service.spec.ts` case for
      ...
      methods (Red).
- [ ] 1.4. Implement `generateAccessGroupDeletionToken` in
      `member.service.ts` (Green).
- [ ] 1.5. Write `member.service.spec.ts` case for
...
- [ ] 1.6. Implement `deleteAccessGroup` in `member.service.ts` (Green).
- [ ] 1.7. Write `member.service.spec.ts` case for
...
- [ ] 1.8. Implement `grantAccessGroupPermission` in `member.service.ts`
      (Green).
- [ ] 1.9. Run `npm run format:check && npm test && npm run build && npm run lint`;
...
- [ ] 2.1. Write `tenant-access-group-management.guard.spec.ts`: caller
      with `TENANT_ACCESS_GROUP_VIEW` in the `GET /api/tenants/permissions`
      response → guard resolves `true` (Red).
- [ ] 2.2. Implement `tenantAccessGroupManagementGuard` in
      `core/tenant-access-group-management.guard.ts` (minimal — permission
      present path only) (Green).
- [ ] 2.3. Write test: permission absent from the response → guard
      resolves a `UrlTree` to `/select-tenant` (Red).
- [ ] 2.4. Extend the guard to redirect on missing permission (Green).
- [ ] 2.5. Write test: `GET /api/tenants/permissions` errors (network
...
- [ ] 2.6. Add the `catchError(() => of(router.parseUrl('/select-tenant')))`
      pipe before the `map` in the guard (Green).
- [ ] 2.7. Write test confirming the guard checks specifically
...
- [ ] 2.8. Run full verification; commit
...
- [ ] 3.1. Write `member-access-group-assignment.component.spec.ts`:
      renders one checkbox per `allGroups` input, pre-checked for ids
      present in `assignedGroupIds` (Red).
- [ ] 3.2. Implement the component's template/inputs (`allGroups`,
      `assignedGroupIds`) (Green).
- [ ] 3.3. Write test: toggling checkboxes and submitting emits
      `submitted` with exactly the checked id set, including the
      single-checked-id case (Red).
- [ ] 3.4. Implement toggle state + `submitted` output (Green).
- [ ] 3.5. Run full verification; commit
...
- [ ] 4.1. Write `tenant-access-group-management-page.component.spec.ts`:
      on init, calls `listAccessGroups(tenantId)` and renders the result
      via `SharedListComponent` (REQ-1) (Red).
- [ ] 4.2. Implement the component shell + group list load (Green).
- [ ] 4.3. Write test: list load 403 → `NoAccessStateComponent` rendered,
      no further requests fired (REQ-2/16 permission-denied branch)
      (Red).
- [ ] 4.4. Implement the 403→`NoAccessStateComponent` branch (Green).
- [ ] 4.5. Write test: list load non-403 failure →
      `ErrorStateComponent` (network branch, REQ-16) (Red).
- [ ] 4.6. Implement the network-error branch (Green).
- [ ] 4.7. Write test: with `TENANT_ACCESS_GROUP_CREATE`, submitting the
      create-group form calls `createAccessGroup` and refreshes the list
      (REQ-4) (Red).
- [ ] 4.8. Implement the create-group form + submit handler (Green).
- [ ] 4.9. Write test: without `TENANT_ACCESS_GROUP_CREATE`, the
      create-group control is not offered/does not submit (REQ-5) (Red).
- [ ] 4.10. Implement the permission gate on the create control (Green).
- [ ] 4.11. Write test: no group selected → roster/detail section is not
      rendered at all (REQ-12) (Red).
- [ ] 4.12. Implement the `selectedGroup()`-gated conditional rendering
      (Green).
- [ ] 4.13. Write test: selecting a group calls `list(tenantId)` once
...
- [ ] 4.14. Implement the roster fetch (`forkJoin` over member details,
      cached in a `Map<membershipId, MemberDetail>`) and the
      roster/candidates `computed()`s (Green).
- [ ] 4.15. Write test: selecting a second group within the same screen
      visit re-filters the cached `memberDetails` map without calling
      `getDetail` again (assert call count unchanged) (Red).
- [ ] 4.16. Implement the cache-reuse path (Green).
- [ ] 4.17. Run full verification; commit
...
- [ ] 5.1. Write test: with `TENANT_PERMISSION_GRANT_CREATE`, submitting
...
- [ ] 5.2. Implement the grant-permission control + submit handler
      (Green).
- [ ] 5.3. Write test: without `TENANT_PERMISSION_GRANT_CREATE`, the
      grant-permission control is not offered/does not submit (part of
      REQ-17's per-action gating) (Red).
- [ ] 5.4. Implement the permission gate on the grant control (Green).
- [ ] 5.5. Write test: grant-permission request 403 →
      `NoAccessStateComponent`/permission-denied outcome, consistent with
      every other gated action (Red).
- [ ] 5.6. Implement the 403 branch for the grant action (Green).
- [ ] 5.7. Run full verification; commit
      `feat(tenant-access-groups): add grant-permission-to-group action`.

## 6. Single assign/unassign (existing confirm-token flow)

- [ ] 6.1. Write test: with `TENANT_PERMISSION_GRANT_CREATE`, choosing a
      ...
      (Red).
- [ ] 6.2. Implement the single-assign path (Green).
- [ ] 6.3. Write test: with `TENANT_PERMISSION_GRANT_DELETE`, unassigning
      ...
      (Red).
- [ ] 6.4. Implement the unassign confirm-dialog round trip (Green).
- [ ] 6.5. Write test: wrong confirmation word on unassign → dialog
      stays open for retry, no `unassignAccessGroup` call made (Red).
- [ ] 6.6. Implement the wrong-word retry path (Green).
- [ ] 6.7. Write test: unassign request 403 → permission-denied outcome,
      roster unchanged (Red).
- [ ] 6.8. Implement the 403 branch for unassign (Green).
- [ ] 6.9. Write test: without `TENANT_PERMISSION_GRANT_CREATE`/
      ...
      (Red).
- [ ] 6.10. Implement the permission gates on assign/unassign controls
      (Green).
- [ ] 6.11. Run full verification; commit
      `feat(tenant-access-groups): add single member assign/unassign`.

## 7. Bulk-assign wiring (`MemberAccessGroupAssignmentComponent`)

- [ ] 7.1. Write test: opening the multi-group assignment entry point for
...
- [ ] 7.2. Implement the entry point + component wiring (Green).
- [ ] 7.3. Write test: a `submitted` event with more than one id calls
...
- [ ] 7.4. Implement the bulk-assign submit handler (Green).
- [ ] 7.5. Write test: a `submitted` event with exactly one id reuses the
...
- [ ] 7.6. Implement the single-vs-batch call selection based on selected
      id count (Green).
- [ ] 7.7. Write test: batch request `400` (partial validation failure)
      → generic/network error branch (not permission-denied), roster
      re-fetched showing only backend-confirmed state (REQ-10) (Red).
- [ ] 7.8. Implement the 400 branch for the batch call (Green).
- [ ] 7.9. Write test: batch request 403 → permission-denied outcome,
      no assignments reflected (REQ-11) (Red).
- [ ] 7.10. Implement the 403 branch for the batch call (Green).
- [ ] 7.11. Run full verification; commit
      `feat(tenant-access-groups): wire multi-group bulk assignment`.

## 8. Delete-group action (new confirm-token flow)

- [ ] 8.1. Write test: with `TENANT_ACCESS_GROUP_DELETE`, requesting
...
- [ ] 8.2. Implement the delete confirm-dialog round trip (Green).
- [ ] 8.3. Write test: successful delete removes the group from the list
      and, if it was the selected group, clears the selection and closes
      the roster/detail section (REQ-14) (Red).
- [ ] 8.4. Implement the post-delete list/selection update (Green).
- [ ] 8.5. Write test: wrong confirmation word on delete → dialog stays
      open for retry, no `deleteAccessGroup` call made (Red).
- [ ] 8.6. Implement the wrong-word retry path (Green).
- [ ] 8.7. Write test: delete request 403 → permission-denied outcome,
      group remains in the list (REQ-15) (Red).
- [ ] 8.8. Implement the 403 branch for delete (Green).
- [ ] 8.9. Write test: without `TENANT_ACCESS_GROUP_DELETE`, the delete
      control is not offered (REQ-17's per-action gating, mirrors 4.9)
      (Red).
- [ ] 8.10. Implement the permission gate on the delete control (Green).
- [ ] 8.11. Run full verification; commit
      `feat(tenant-access-groups): add delete-group action`.

## 9. Route registration

- [ ] 9.1. Write/extend `app.routes.spec.ts` (or the closest existing
...
- [ ] 9.2. Add the route to `app.routes.ts` per PLAN.md's exact
      `loadComponent`/`canActivate` block (Green).
- [ ] 9.3. Run full verification; commit
      `feat(routes): register /tenants/access-groups route`.

## 10. Nav-menu entry

- [ ] 10.1. Write/extend `nav-menu.component.spec.ts`: the tenant
...
- [ ] 10.2. Add the nav-menu entry (link + icon + gating) in
      `nav-menu.component.ts` (Green).
- [ ] 10.3. Run full verification; commit
      `feat(nav-menu): add tenant Access groups link (TENANT_ACCESS_GROUP_VIEW)`.

## 11. Final pass

- [ ] 11.1. Full-repo search for any leftover TODO/stub related to this
      feature (e.g. an unused import from an earlier deviation); fix any
      found.
- [ ] 11.2. Run `npm run format`, then
      `npm run format:check && npm test && npm run build && npm run lint`
      across the whole subproject; confirm all green.
- [ ] 11.3. Update `PLAN.md` if any decision changed during
      ...
      PLAN" entries).
- [ ] 11.4. Update `PROJECT_STATUS.md` with what shipped (new screen,
