...
- [ ] 1. Add `PROFILE_EDIT` to `core/permission.ts` (`Permission` union +
      `ALL_PERMISSIONS`) and `core/global-permission.ts`
      (`GlobalPermission` union + `ALL_GLOBAL_PERMISSIONS`).
- [ ] 2. Write `profile.service.spec.ts` covering `getOwnProfile()` →
      `GET /api/users/me/profile` (Red).
- [ ] 3. Create `core/profile.service.ts` with `ProfileFields`,
      `UserProfile`, `ProfileEditRequest`, `ProfileEditRequestStatus`
      types and `getOwnProfile()` (Green).
- [ ] 4. Extend `profile.service.spec.ts` for `getProfile(userId)` →
      `GET /api/users/{id}/profile` (Red); implement (Green).
- [ ] 5. Extend `profile.service.spec.ts` for `directEdit(userId,
      fields)` → `PUT /api/users/{id}/profile` (Red); implement
      (Green).
- [ ] 6. Extend `profile.service.spec.ts` for `submitEditRequest(fields)`
      → `POST /api/users/me/profile/edit-requests` (Red); implement
      (Green).
- [ ] 7. Extend `profile.service.spec.ts` for `listEditRequests()` →
      `GET /api/profile-edit-requests` (Red); implement (Green).
- [ ] 8. Extend `profile.service.spec.ts` for `approveEditRequest(id)` /
      `rejectEditRequest(id)` → their respective `POST .../approve` /
      `.../reject` calls (Red); implement (Green).

## Shared form component

- [ ] 9. Write `profile-fields-form.component.spec.ts`: renders
      `fullName`/`address`/`rg`/`cpf`/`phone` inputs, never an `email`
      input (Red).
- [ ] 10. Create `shared/profile-fields-form.component.ts` implementing
      the above (Green).
- [ ] 11. Extend the spec: submitting the form emits `submitted` with
      the entered values, never including `email` in the emitted object
      (Red); implement (Green).
- [ ] 12. Extend the spec: `[disabled]=true` prevents submission/emits
      nothing (Red); implement (Green).

## Own-profile screen

- [ ] 13. Write `own-profile-page.component.spec.ts`: on init, calls
      `GET /api/users/me/profile` and renders the returned fields with
      `email` shown read-only (Red).
- [ ] 14. Create `features/profile/own-profile-page.component.ts`
      implementing the load + render (Green).
- [ ] 15. Extend the spec: a `STAFF_ADMIN`-shaped session (all global
...
- [ ] 26. Extend the spec: a 409 on that call shows the conflict
      message (REQ-11) (Red); implement (Green).
- [ ] 27. Extend the spec: `[canEdit]=false` never renders the edit
      toggle (Red); implement the gate (Green).
- [ ] 28. Write a new case in `staff-user-detail-panel.component.spec.ts`:
...
- [ ] 29. Write a new case in `member-detail-panel.component.spec.ts`:
...
- [ ] 30. Write `profile-edit-requests-inbox-page.component.spec.ts`: on
...
- [ ] 39. Extend `nav-menu.component.spec.ts`: the edit-request inbox
...
- [ ] 40. Extend `nav-menu.component.spec.ts`: the edit-request inbox
...
- [ ] 41. Run `npm run format:check && npm test && npm run build` and
      confirm everything is green.
- [ ] 42. Update `PLAN.md` if any decision changed during
      implementation (per this repo's standing convention — see
      `staff-global-dashboard/PLAN.md`'s own precedent for this kind of
      note, if applicable).
- [ ] 43. Update `PROJECT_STATUS.md`: mark `user-profile` (frontend
