...
- [x] 1.1. Add `permissions.descriptions.<ENUM>` keys to
...
- [x] 1.2. Add the same keys, translated to Portuguese (pt-BR), to
...
- [x] 1.3. `npm run format` on both JSON files; commit
      (`feat(role-permission-management-ui): add permission description
      i18n keys`).

## 2. `translatePermissionDescription` helper

- [x] 2.1. **Red**: add a test (new `permission-labels.spec.ts` if none
...
- [x] 2.2. **Green**: implement `translatePermissionDescription` in
...
- [x] 3.3. **Green**: scaffold `PermissionListComponent` (`rows: input<
...
- [x] 3.4. **Red**: extend the spec — `editable` mode renders a
...
- [x] 3.5. **Green**: add the switch control (reusing the existing
...
- [x] 3.6. **Red**: extend the spec — clicking a switch, and pressing
...
- [x] 3.7. **Green**: add the `toggle = output<string>()` and click/
      keydown handlers emitting it, no local state mutation. Run
      `npm test -- permission-list`.
      (Later renamed `toggle` → `permissionToggle`, see PLAN.md's
      "Deviations" section — `@angular-eslint/no-output-native`.)
- [x] 3.8. `npm run format`; commit (`feat(role-permission-management-ui):
      add reusable PermissionListComponent`).

## 4. `member-detail-panel.component.ts` — tabs + `PermissionListComponent`

- [x] 4.1. **Red**: in `member-detail-panel.component.spec.ts`, add
...
- [x] 4.2. **Green**: add the `activeTab = signal<'personal' |
...
- [x] 4.3. **Red**: extend the spec — the old inline toggle-grid markup
      ...
      buttons.
- [x] 4.4. **Green**: move `direct-permissions` section's toggle grid into
...
- [x] 4.5. `npm run format`; commit (`feat(role-permission-management-ui):
...
- [x] 5.1. Read the current file's `viewMode`/History-Edit structure
      (commit 44e70fc) before editing, to confirm the exact branch to
      nest inside.
- [x] 5.2. **Red**: add a regression test asserting that when
...
- [x] 5.3. **Green**: confirm (no code change expected) this already
...
- [x] 5.5. **Green**: add the same `activeTab`/`tabClass`/`selectTab`/
      ...
      `npm test -- staff-user-detail-panel`.
- [x] 5.6. **Red**: extend the spec — the old direct-`GlobalPermission`
...
- [x] 6.2. **Green**: add `revokeAccessGroupPermission` to
      `MemberService` (confirmed request/path shape against
      `knowly-api/specify/features/role-permission-revoke/PLAN.md`)
      and extend the `AccessGroup` interface with
      `permissions: Permission[]`. Run `npm test -- member.service`.
- [x] 6.3. **Red**: `staff-user.service.spec.ts` — same test shape for
...
- [x] 6.4. **Green**: add the method + interface field to
      `StaffUserService`. Run `npm test -- staff-user.service`.
- [x] 6.5. `npm run format`; commit (`feat(role-permission-management-ui):
      add access-group permission revoke methods`).

## 7. Tenant roles page — replace grant-only control with `PermissionListComponent`

- [x] 7.1. **Red**: `tenant-access-group-management-page.component.spec.ts`
...
- [x] 7.2. **Green**: delete `grant-permission-form` from
...
- [x] 7.3. **Red**: extend the spec — a failed grant/revoke call leaves
...
- [x] 7.4. **Green**: implement the optimistic-toggle-with-rollback
      handler per PLAN — flip `groupPermissions` immediately, revert on
      non-2xx, set `permissionActionError`. Run
      `npm test -- tenant-access-group-management-page`.
- [x] 7.5. **Red**: extend the spec — a second click on the same row
...
- [x] 7.6. **Green**: add the page-level `pendingPermissionToggles`
      in-flight guard per PLAN (list-wide disable, see PLAN's
      "Deviations" section for why not a strict per-row disabled set),
      adding/removing the row's permission value around the HTTP call
      and gating the permission-list's `[disabled]` input on it. Run
      `npm test -- tenant-access-group-management-page`.
- [x] 7.7. `npm run format`; commit (`feat(role-permission-management-ui):
      replace tenant role grant control with PermissionListComponent`).

## 8. Staff/global roles page — new permission-editing section

- [x] 8.1. **Red**: `access-group-management-page.component.spec.ts` —
      ...
      `revokeAccessGroupPermission`.
- [x] 8.2. **Green**: add `<app-permission-list>` to
...
- [x] 8.3. **Red**: extend the spec — same failed-call rollback +
      inline `permissionActionError` assertion as task 7.3, scoped to
      this page.
- [x] 8.4. **Green**: implement the same optimistic rollback handler. Run
      `npm test -- access-group-management-page`.
- [x] 8.5. **Red**: extend the spec — same in-flight double-click guard
      assertion as task 7.5, scoped to this page.
- [x] 8.6. **Green**: implement the same `pendingPermissionToggles`
      in-flight guard. Run `npm test -- access-group-management-page`.
- [x] 8.7. `npm run format`; commit (`feat(role-permission-management-ui):
      add permission editing to staff/global roles page`).

## 9. Full verification and PLAN sync

- [x] 9.1. Run `npm run format:check && npm test && npm run build &&
      npm run lint`; fix anything red. (Found and fixed one lint error:
      `@angular-eslint/no-output-native` on `PermissionListComponent`'s
      `toggle` output — renamed to `permissionToggle`, see task 3.7's
      note and PLAN.md's "Deviations" section.)
- [x] 9.2. Update `PLAN.md`'s "Testing strategy"/other sections — see
      its new "Deviations from this PLAN" section for the four
      decisions that changed during implementation (output rename,
      `disabled` input, `Permission` enum gap fix, defensive `?? []`).
- [x] 9.3. Commit any final fixups
      (`fix(role-permission-management-ui): ...`).
