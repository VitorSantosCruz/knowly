...
describe('ExportButtonComponent', () => {
  ...
  it('issues a GET with responseType blob and the current period on activation', () => {
    const button: HTMLElement = fixture.nativeElement.querySelector('[data-testid="export-button"]');
    button.click();
  ...
  it('shows an inline error on a failing export request', () => {
    ...
    httpMock.expectOne((request) => request.url === '/api/tenants/metrics/export').flush(
      new Blob(['boom']),
      {
        status: 500,
