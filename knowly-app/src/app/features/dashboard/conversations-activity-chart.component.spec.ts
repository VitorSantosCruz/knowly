...
import {
  ...
  toBarData,
} from './conversations-activity-chart.component';
...
describe('toBarData', () => {
  ...
  it('returns an empty shape for no days', () => {
    expect(toBarData({ days: [] })).toEqual({ labels: [], datasets: [{ data: [] }] });
  });
});

@Component({
  selector: 'app-host',
  imports: [ConversationsActivityChartComponent],
  template: `<app-conversations-activity-chart [period]="period()" />`,
})
class HostComponent {
  readonly period = signal<Period>('30d');
}

describe('ConversationsActivityChartComponent', () => {
