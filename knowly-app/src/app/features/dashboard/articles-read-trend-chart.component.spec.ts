...
import {
  ...
  toArticlesReadChartData,
} from './articles-read-trend-chart.component';
import { DailyCountRow } from './trend-chart-data';

describe('toArticlesReadChartData', () => {
  it('maps day/count rows to a labels/datasets shape', () => {
    const result = toArticlesReadChartData([
  ...
  it('returns an empty shape for no rows', () => {
    expect(toArticlesReadChartData([])).toEqual({ labels: [], datasets: [{ data: [] }] });
  });
});

@Component({
  selector: 'app-host',
  imports: [ArticlesReadTrendChartComponent],
  template: `<app-articles-read-trend-chart [data]="data" [error]="error" />`,
})
class HostComponent {
  data: DailyCountRow[] = [];
  error = false;
...
describe('ArticlesReadTrendChartComponent', () => {
