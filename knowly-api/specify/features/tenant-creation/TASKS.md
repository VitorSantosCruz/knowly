...
- [x] 12. `TaxIdNormalizer` (new,
      `br.com.conectabyte.knowly.tenancy.validation`, public -- not
      package-private as originally planned, see PLAN.md's
      "Deviations"): `normalize(String)` strips `.`/`-`/`/`. Test
...
- [x] 13. `CnpjChecksumValidator` (new, same package, public for the
      same reason): `isValid(String normalizedTaxId)` implementing the
      two-check-digit mod-11 algorithm (PLAN.md's weight sequences,
      corrected during implementation -- see PLAN.md's "Deviations" --
      and alphanumeric-value adjustment). Test
...
- [x] 14. `TaxIdValidator.isValid` updated to call
...
- [x] 15. `InvalidTaxIdException` (new,
...
- [x] 16. `TenantService#createTenant` rewritten per PLAN.md's corrected
...
- [x] 17. Integration regression test (new focused test class or an
...
- [x] 18. Full acceptance-criteria pass: re-verify every new checkbox
      ...
      **Deviation found during this pass:** the stricter REQ-6b shape
      check (exactly 14 characters total, not "contains 14 digits
      somewhere") broke two pre-existing, unrelated test fixtures that
      used a placeholder `"TAXID" + System.nanoTime()` value —
      `StaffRbacIntegrationTest` and `TenantManagementIntegrationTest`.
      The old, looser `TaxIdValidator` stripped *all* non-digit
      characters and only checked the resulting digit *count* (`== 14`),
      so a string like `"TAXID68619882341307"` (5 letters + 14 digits =
      19 characters) passed by accident. The new shape check correctly
      rejects it (19 characters total, not 14). Both files gained a
      `validCnpj()`/`checkDigit(...)` test helper (mirrors
      `CnpjChecksumValidator`'s algorithm, generates a checksum-valid,
      effectively-unique CNPJ per call from `System.nanoTime()`) to
      replace the placeholder — not a scope change, a fix to test
      fixtures that were exploiting a bug in the code this amendment
      corrects.
- [x] 19. Run `./mvnw spotless:apply && ./mvnw verify` and confirm the
      full suite is green before committing.
