...
- [x] 1. **Red** — Write `TenantRepositoryTest` cases: seed `Tenant` rows
...
- [x] 2. **Green** — Add `TenantRepository.countCumulativeTenantsByDay():
      List<DailyCountProjection>` (native `@Query`, window-function SQL
      per PLAN.md's "Architectural decisions").
- [x] 3. Run `./mvnw test -Dtest=TenantRepositoryTest` and confirm green;
      commit (`feat(metrics): add cumulative tenant count query`).
- [x] 4. **Red** — Write `UserRepositoryTest` cases: seed `User` rows with
...
- [x] 5. **Green** — Add `UserRepository.countCumulativeStaffByDay():
      List<DailyCountProjection>` (native `@Query`, same shape, `WHERE
      global_role IN ('STAFF','STAFF_ADMIN')`).
- [x] 6. Run `./mvnw test -Dtest=UserRepositoryTest` and confirm green;
      commit (`feat(metrics): add cumulative staff count query`).

## 2. `GlobalTrendsDto` shape (REQ-1)

- [x] 7. Append `totalTenantsPerDay`/`staffCountPerDay` fields to
...
- [x] 8. **Red** — Write `GlobalMetricsServiceTest` cases for a new
      ...
      - Bounded period with zero rows at all: every day in the range is
        `0`.
- [x] 9. **Green** — Implement `mergeCarryForwardDays(List<DailyCountProjection>,
      MetricsPeriod)` as a private helper on `GlobalMetricsService`,
      exactly per PLAN.md's algorithm.
- [x] 10. **Red** — Write `GlobalMetricsServiceTest` cases confirming
...
- [x] 14. **Green** — No production code expected to change here if
      ...
      step surfaces.
- [x] 15. Run `./mvnw test -Dtest=GlobalMetricsControllerIntegrationTest`
      and confirm green; commit
      (`test(metrics): cover cumulative trend series at the API boundary`).

## 5. Full regression + doc sync

- [x] 16. Run `./mvnw spotless:apply && ./mvnw verify` for the whole
...
- [x] 17. Update `PLAN.md` with any decision that changed during
      implementation.
- [x] 18. Update root `PROJECT_STATUS.md` to record
      `global-staff-dashboard-sparklines` (backend) as implemented.
- [x] 19. Final commit for any doc-only changes from steps 17/18
