...
- [x] 1. Write `PermissionAspectTest`: `MEMBER_ADMIN` (active membership,
      ...
      branch).
- [x] 2. Add the `membership.getRole() == MembershipRole.MEMBER_ADMIN`
...
- [x] 3. Write `PermissionAspectTest`: same `MEMBER_ADMIN` user, active
      ...
      wrong).
- [x] 4. Confirm task 3 passes without further code change (Green) — if
      it doesn't, fix the bypass scoping (should never need a second
      lookup or a client-supplied tenant id).
- [x] 5. Write `PermissionAspectTest`: `MEMBER_ADMIN` role present but the
      ...
      code).
- [x] 6. Confirm task 5 passes (Green, no code change expected).
- [x] 7. Run existing `PermissionAspectTest` `STAFF_ADMIN` case(s) and
      confirm unchanged (regression check, acceptance criterion 7).

## REQ-4/REQ-5 — `TenantService` self-escalation guard

- [x] 8. Write `TenantServiceTest`: `addMember(actor, tenantId,
...
- [x] 9. Add `private void requireNotSelfTarget(User actor, Long
...
- [x] 10. Write `TenantServiceTest`: `grantPermission(actor, tenantId,
      actor's-own-membershipId, permission)` where actor is
      `MEMBER_ADMIN` of `tenantId` → expect `PermissionDeniedException`
      (Red).
- [x] 11. Call `requireNotSelfTarget(actor, membership.getUser().getId())`
...
- [x] 12. Repeat tasks 10-11's Red/Green pair for `revokePermission`
      (same self-membershipId shape).
- [x] 13. Repeat tasks 10-11's Red/Green pair for `assignAccessGroup`.
- [x] 14. Repeat tasks 10-11's Red/Green pair for `unassignAccessGroup`.
- [x] 15. Write `TenantServiceTest`: each of the five methods
...
- [x] 16. Write a test (extend `AuditLogAspectTest` or add a
...
- [x] 17. Confirm task 16 passes with no production code change (Green).

## Regression and wrap-up

- [x] 18. Run existing `UserProfileServiceTest` and
...
- [x] 19. Run the full `./mvnw spotless:apply` then `./mvnw verify` and
      confirm the whole suite is green.
- [x] 20. Update `PROJECT_STATUS.md` to reflect this feature's
