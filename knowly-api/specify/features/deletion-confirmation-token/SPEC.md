...
- [ ] A generic mechanism exists for generating and validating deletion
      confirmation tokens, keyed by resource type + resource instance id
      + requesting user/session.
- [ ] A generated token's word is composed of exactly two distinct,
...
- [ ] Both an EN wordlist and a pt-BR wordlist exist in-repo at launch,
      ...
      trade-off.
- [ ] When a token generation request's `Accept-Language` header
...
- [ ] A token expires 5 minutes after generation by default, and this
      duration is configurable.
- [ ] A token is invalidated after a single successful use.
- [ ] A token cannot be used to delete a resource instance other than
      the one it was generated for.
- [ ] A token generated by one user cannot be used by another user.
- [ ] An expired or already-used token is rejected the same way as a
      wrong word (no distinguishing information in the response).
- [ ] Requesting a new token for a resource instance that already has a
      live token from the same user invalidates the previous one.
- [ ] `DELETE /api/tenants/{tenantId}/articles/{articleId}` requires a
      valid confirmation token scoped to that article and caller, and
      rejects the deletion without one.
- [ ] A new endpoint lets a caller with `ARTICLE_DELETE` permission
      generate a confirmation token for a specific article; callers
      without that permission cannot.
- [ ] `DELETE /api/tenants/{tenantId}/members/{membershipId}` requires a
...
- [ ] `DELETE /api/users/{userId}/permissions/{permission}` requires a
...
- [ ] `DELETE /api/users/{userId}/access-groups/{accessGroupId}`
...
- [ ] Token generation and validation (success and failure) are
