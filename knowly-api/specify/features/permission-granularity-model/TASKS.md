...
- [ ] 1. Write a unit test for `Permission#viewDependency()` enumerating
      every expected pair (`ARTICLE_EDIT`→`ARTICLE_VIEW`,
      `ARTICLE_DELETE`→`ARTICLE_VIEW`) and every other current value
      returning empty (Red).
- [ ] 2. Add `viewDependency()` to `Permission` implementing task 1's
      expectations (Green).
- [ ] 3. Write a unit test for `GlobalPermission#viewDependency()`
...
- [ ] 4. Add the new `GlobalPermission` values (`TENANT_VIEW`,
...
- [ ] 5. Write `PermissionAspectTest` cases for REQ-2/REQ-5 on
...
- [ ] 9. Write a `TenantService` unit/integration test: a `STAFF` user
      ...
      updated).
- [ ] 10. Update `TenantService#requireAdminOfTenantOrStaff` to also
      ...
      task 9).
- [ ] 11. Extend `StaffRbacIntegrationTest` with the remaining
...
- [ ] 12. Write `ArticleControllerIntegrationTest` (or extend the
...
- [ ] 13. Write a `V23MigrationIntegrationTest` per PLAN.md's testing
      ...
      NOTHING`), then run `V23`, then assert via raw JDBC that every
      expected granular row exists exactly once and every bundled row is
      gone (Red — migration doesn't exist yet).
- [ ] 14. Write `V23__expand_bundled_global_permissions.sql` per
      PLAN.md's pattern (Green for task 13).
- [ ] 15. Remove `TENANT_MEMBER_MANAGE_ANY`, `TENANT_ACCESS_GROUP_MANAGE_ANY`,
      `TENANT_PERMISSION_GRANT_MANAGE_ANY` from `GlobalPermission` now
      that no code references them and `V23` has run in every
      ...
      site).
- [ ] 16. Run the full `./mvnw verify` and confirm the suite is green.
- [ ] 17. Update `PLAN.md`/root `DECISIONS.md` if any decision changed
      during implementation (in particular, if the `V23` migration test
