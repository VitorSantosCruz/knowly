...
- [x] 1. Add `src/main/resources/db/migration/V29__access_group_soft_delete.sql`
...
- [x] 2. Test: an `AccessGroupRepositoryTest`/`@DataJpaTest` case
...
- [x] 7. Implement `AccessGroupRepository#findByTenantAndIdInAndDeletedAtIsNull`
      (Green).
- [x] 8. Test: `AccessGroupRepositoryTest#findByIdAndDeletedAtIsNull` —
      returns the row when live, empty when soft-deleted (Red).
- [x] 9. Implement `AccessGroupRepository#findByIdAndDeletedAtIsNull`
      (Green).
- [x] 10. Test: `AccessGroupRepositoryTest#findByTenantAndDeletedAtIsNull`
      — excludes soft-deleted rows for the tenant, replacing the
      existing `findByTenant` call site's semantics (Red).
- [x] 11. Implement `AccessGroupRepository#findByTenantAndDeletedAtIsNull`
      (Green — do not remove `findByTenant` yet; the `listAccessGroups`
      call-site switch happens in section 6).
- [x] 12. Test: `AccessGroupPermissionRepositoryTest#findByAccessGroupInAndDeletedAtIsNull`
      — given a list of `AccessGroup`s, excludes soft-deleted permission
      rows for those groups (Red).
- [x] 13. Implement `AccessGroupPermissionRepository#findByAccessGroupInAndDeletedAtIsNull`
      (Green).
- [x] 14. Test: `AccessGroupPermissionRepositoryTest` — a new
      ...
      (Red).
- [x] 15. Implement the bulk `@Modifying @Query` soft-delete method on
      `AccessGroupPermissionRepository` (Green).
- [x] 16. Test: `UserAccessGroupRepositoryTest` — same shape as task 14,
      for a new bulk soft-delete method scoped by `accessGroupId` on
      `UserAccessGroupRepository` (Red).
- [x] 17. Implement the bulk `@Modifying @Query` soft-delete method on
      `UserAccessGroupRepository` (Green).

## 3. New exceptions (REQ-3, REQ-15)

- [x] 18. Test: `TenancyExceptionHandlerTest` (or equivalent) — throwing
...
- [x] 19. Implement `exception/AccessGroupNotFoundException.java` +
      its `@ExceptionHandler` entry (404) in
      `exception/TenancyExceptionHandler.java` (Green).
- [x] 20. Test: same shape as task 18, for `InvalidAccessGroupBatchException`
      → 400, code `INVALID_ACCESS_GROUP_BATCH` (Red).
- [x] 21. Implement `exception/InvalidAccessGroupBatchException.java` +
      its `@ExceptionHandler` entry (400) in `TenancyExceptionHandler.java`
      (Green).

## 4. New DTO (REQ-1, REQ-4, AppSec finding)

- [x] 22. Test: `BatchAccessGroupAssignmentRequestDtoTest` (bean
...
- [x] 26. Test: `TenantServiceTest#batchAssignAccessGroups` — happy path
...
- [x] 30. Test: `TenantControllerIntegrationTest` — `POST
...
- [x] 31. Implement the `POST .../access-groups:batch` handler on
...
- [x] 32. Test: `TenantServiceTest#generateAccessGroupDeletionConfirmationToken`
      ...
      (Red).
- [x] 33. Implement `TenantService#generateAccessGroupDeletionConfirmationToken`
...
- [x] 34. Test: `TenantControllerIntegrationTest` — `GET
...
- [x] 35. Implement the `GET .../deletion-confirmation-token` handler on
...
- [x] 36. Test: `TenantServiceTest#deleteAccessGroup` — happy path with
...
- [x] 40. Implement `TenantService#deleteAccessGroup` (permission check
...
- [x] 41. Test: `TenantServiceTest#deleteAccessGroup` — a forced
...
- [x] 42. Confirm `@Transactional` boundary on `deleteAccessGroup`
...
- [x] 43. Test: `TenantControllerIntegrationTest` — `DELETE
      ...
      `accessGroupId` (Red).
- [x] 44. Implement the `DELETE .../access-groups/{accessGroupId}`
...
- [x] 45. Test: `TenantControllerIntegrationTest` — a staff caller
      ...
      (Red).
- [x] 46. Confirm/adjust the guards from tasks 29/33/40 already satisfy
...
- [x] 47. Test: `TenantServiceTest#listAccessGroups` — excludes a
      soft-deleted `AccessGroup` from the result (Red).
- [x] 48. Update `TenantService#listAccessGroups`'s call site from
      `AccessGroupRepository#findByTenant` to
      `findByTenantAndDeletedAtIsNull` (Green).
- [x] 49. Test: `TenantServiceTest#grantAccessGroupPermission` — a
...
- [x] 50. Update `TenantService#grantAccessGroupPermission`'s
      `AccessGroupRepository#findById` call site to
      `findByIdAndDeletedAtIsNull` (Green).
- [x] 51. Test: `TenantServiceTest#assignAccessGroup` — same shape as
      task 49, for the existing single-assign method (Red).
- [x] 52. Update `TenantService#assignAccessGroup`'s
      `AccessGroupRepository#findById` call site to
      `findByIdAndDeletedAtIsNull` (Green).
- [x] 53. Test: `PermissionServiceTest` (effective-permission
...
- [x] 56. Confirm task 55 passes with no further code change (Green —
...
- [x] 57. Test: `TenantServiceTest` — after `deleteAccessGroup` soft-deletes
      ...
      (Red).
- [x] 58. Confirm task 57 passes with no further code change (Green —
      this is a regression check on the migration/entity work from
      sections 0-1; fix here if a gap is found).

## 14. Final verification

- [x] 59. Run the full `./mvnw spotless:apply && ./mvnw verify` and
      confirm the entire suite is green.
- [x] 60. Update `PLAN.md` if any decision changed during implementation.
- [x] 61. Update `SPEC.md`'s acceptance-criteria checkboxes to reflect
      what's now verified by tests.
- [x] 62. Update `../../../../PROJECT_STATUS.md` to reflect the new
